 ...
 *
 *
 *		Menu module controls the information displayed on the screen and defines multiple
 * views that can be shown to the user. During program execution cycle the first thing that
 ...
 *
 * 		Text on the screen is defined with text fields that contain the char array address
 * and the x and y coordinates of a single text field. A single menu view is defined with
 * an array of text fields and the amount of text fields in it.
 *
 * 		When current menu view is changed to another the menu system changes it's current
 * text fields according to new view's fields. Because some text fields are updatable, such
 * as the ones showing newest measurement results and the ones indicating selection state,
...
/*
 * Updates current textfields similar to newly chosen view's textfields.
 */
